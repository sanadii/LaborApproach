...
import {
    Container, Button, UncontrolledTooltip, Input,
    DropdownToggle, DropdownMenu, Dropdown, DropdownItem,
    Row, Col, Card, CardBody, UncontrolledDropdown,
...
import { getChatRooms } from "store/actions";
...
const ChatRooms = () => {
    const { chatRooms, messages } = useSelector(chatSelector);
    ...
    const { sendMessage, lastMessage } = useWebSocket(socketUrl, {
        shouldReconnect: (closeEvent) => true, // Add reconnect logic if needed
    ...
    //Use For Chat Box
    const userChatOpen = (id, name, status, roomUuid, image) => {
        console.log("id", id, "name", name, "status", status, "roomUuid", roomUuid, "image", image);
        joinChat(roomUuid);
        setIsInChatRoom(true);
        setIsChatOpen(true);
                                        ...
                                        >
                                            {(chatRooms || []).map((chat) => (
                                                <li
                                                    ...
                                                    className={
                                                        currentRoomId === chat.roomUuid ? "active" : ""
                                                    }
                                                >
                                                    <Link
                                                        to="#"
                                                    ...
                                                    >
                                                        <div className="d-flex align-items-center">
                                                            ...
                                                            <div className="flex-grow-1 overflow-hidden">
                                                                <p className="text-truncate mb-0">{chat.status}</p>
                                                                {/* <p className="text-truncate mb-0">{chat.client}</p> */}
                                                            </div>
